...
if 1 in sections:
    ...
    path_af_weights = './3_output_cifar/p_ASC_main/0_af_weights/'
    path_all_weights = './3_output_cifar/p_ASC_main/0_all_weights/'
    layer_list = ['c1', 'c2', 'c3', 'c4', 'd1', 'd2']

    # extract weights from files
    ts_tanh, alphas_tanh, _ = smcn_extract_af_weights_over_time(path_af_weights, 'alpha_tanh_run', 60000, 60)
    print('AF weights extracted for tanh')
    ts_relu, alphas_relu, _ = smcn_extract_af_weights_over_time(path_af_weights, 'alpha_relu_run', 60000, 60)
    print('AF weights extracted for relu')
    ts_elu, alphas_elu, _ = smcn_extract_af_weights_over_time(path_af_weights, 'alpha_elu_run', 60000, 60)
    print('AF weights extracted for elu')

    # plot weights over time
    alphas_list = [alphas_tanh, alphas_relu, alphas_elu]
    af_name_list = [r'$\alpha tanh$', r'$\alpha ReLU$', r'$\alpha ELU$']
    ts_list = [ts_tanh, ts_relu, ts_elu]
    ylim_list = [[0.,1.2], [0.,2.0], [0.,4.0]]
    plot_mean_alpha_by_layers_over_time_grouped(alphas_list, af_name_list, ts_list, layer_list, './3_result_plots/', 'P_MAIN_ASU_over_time', ylim_list=ylim_list)
...
if 2 in sections:
    ...
    ts_b5u, alphas_b5u, betas_b5u = smcn_extract_af_weights_over_time(path_af_weights, 'ABU_TERIS_run', 60000, 60)
    print('AF weights extracted for ABU')
    ts_b5n, alphas_b5n, betas_b5n = smcn_extract_af_weights_over_time(path_af_weights, 'ABU_N_TERIS_run', 60000, 60)
    print('AF weights extracted for ABU_N')
    ts_b5p, alphas_b5p, betas_b5p = smcn_extract_af_weights_over_time(path_af_weights, 'ABU_P_TERIS_run', 60000, 60)
    print('AF weights extracted for ABU_P')
    ts_b5a, alphas_b5a, betas_b5a = smcn_extract_af_weights_over_time(path_af_weights, 'ABU_A_TERIS_run', 60000, 60)
    print('AF weights extracted for ABU_A')
    ts_b5s, alphas_b5s, betas_b5s = smcn_extract_af_weights_over_time(path_af_weights, 'ABU_S_TERIS_run', 60000, 60)
    print('AF weights extracted for ABU_S')

    plot_mean_ABU_alphas_over_time(alphas_b5u, ts_b5u, layer_list, abu_af_list, r'$ABU$'+': mean '+r'$\alpha_{ij}$', './3_result_plots/', 'P_over_time_ABU_alphas_default_init', ylim=[-.6,.6], norm="None")
    plot_mean_ABU_alphas_over_time(alphas_b5n, ts_b5n, layer_list, abu_af_list, r'$ABU_N$'+': mean '+r'$\alpha_{ij}$', './3_result_plots/', 'P_over_time_ABU_N_alphas_default_init', ylim=[-2.,2.4], norm="N")
    plot_mean_ABU_alphas_over_time(alphas_b5p, ts_b5p, layer_list, abu_af_list, r'$ABU_P$'+': mean '+r'$\alpha_{ij}$', './3_result_plots/', 'P_over_time_ABU_P_alphas_default_init', ylim=[-.1,1.1], norm="P")
    plot_mean_ABU_alphas_over_time(alphas_b5a, ts_b5a, layer_list, abu_af_list, r'$ABU_A$'+': mean '+r'$\alpha_{ij}$', './3_result_plots/', 'P_over_time_ABU_A_alphas_default_init', ylim=[-.4,1.], norm="A")
    plot_mean_ABU_alphas_over_time(alphas_b5s, ts_b5s, layer_list, abu_af_list, r'$ABU_S$'+': mean '+r'$\alpha_{ij}$', './3_result_plots/', 'P_over_time_ABU_S_alphas_default_init', ylim=[-.1,1.1], norm="S")
