...
def test(TaskSettings, Paths, Network, test_handler, print_results=False, print_messages=True):
	...
	with tf.Session(config = config) as sess:
		...
		while test_handler.test_mb_counter < test_handler.n_test_minibatches:
			...
			loss, top1, logits = sess.run([Network.loss, Network.top1, Network.logits], feed_dict = { Network.X: test_imageBatch, Network.Y: test_labelBatch, Network.dropout_keep_prob: .5})
			# STORE PERFORMANCE
			loss_store.append(loss)
			top1_store.append(top1)
