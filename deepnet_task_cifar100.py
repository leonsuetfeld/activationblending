...
class Paths(object):
	...
	def __init__(self, TaskSettings):
		# data location
		self.train_set = './1_data_cifar100/train_set/'
		self.test_set = './1_data_cifar100/test_set/'
		self.train_set_ztrans = './1_data_cifar100/train_set_ztrans/'
		self.test_set_ztrans = './1_data_cifar100/test_set_ztrans/'
		self.train_set_gcn_zca = './1_data_cifar100/train_set_gcn_zca/'
		self.test_set_gcn_zca = './1_data_cifar100/test_set_gcn_zca/'
...
def test_saved_model(TaskSettings, Paths, Network, test_handler, print_results=False, print_messages=True):
	...
	with tf.Session(config = config) as sess:
		...
		while test_handler.test_mb_counter < test_handler.n_test_minibatches:
			...
			test_imageBatch, test_labelBatch = test_handler.create_next_test_minibatch()
			loss, top1, logits = sess.run([Network.loss, Network.top1, Network.logits], feed_dict = { Network.X: test_imageBatch, Network.Y: test_labelBatch, Network.dropout_keep_prob: TaskSettings.dropout_keep_probs_inference})
			# STORE PERFORMANCE
			loss_store.append(loss)
			top1_store.append(top1)
		...
		# TODO: implement for 100 classes
		if print_results:
			print('=================================================================================================================================================================================================')
